...
static bool link_mode7_program(SnesRecompPresenter *presenter) {
    ...
    static const char fragment_source[] =
        ...
        "  int tile = int(byte_value(texelFetch(map_texture,\n"
        "      tile_xy, 0).r));\n"
        ...
        "}\n"
        "/* Nearest by default. Interpolating takes the plane between two\n"
        "   native scanlines instead of repeating one, but only where they\n"
        "   describe the same surface: near the horizon consecutive rows can\n"
        "   sit further apart than the shortest wrap, and blending those\n"
        "   would invent geometry. */\n"
        "vec4 affine_row(float linef) {\n"
        "  int l0 = clamp(int(floor(linef)), 0, int(native_height) - 1);\n"
        "  vec4 a0 = texelFetch(line_texture, ivec2(0, l0), 0);\n"
        "  if (line_lerp < 0.5) return a0;\n"
        "  int l1 = clamp(l0 + 1, 0, int(native_height) - 1);\n"
        "  vec4 a1 = texelFetch(line_texture, ivec2(0, l1), 0);\n"
        "  vec2 d = a1.xy - a0.xy;\n"
        "  d -= map_fixed_wrap * floor(d / map_fixed_wrap + 0.5);\n"
        ...
        "    /* Tile numbers and palette indices cannot be blended, so every\n"
        "       tap is resolved to colour first. Index zero is transparent and\n"
        "       is left out rather than bleeding backdrop into the edge, and\n"
        "       the nearest tap still decides priority and colour math -- only\n"
...
static bool opengl_present_mode7_hd(
    ...
    glUniform1f(glGetUniformLocation(context->mode7_program, "line_lerp"),
                frame->interpolate_lines ? 1.0f : 0.0f);
